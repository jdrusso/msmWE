...
class modelWE():
    ...
    def initialize(self,fileSpecifier,refPDBfile,initPDBfile,modelName):
        ...
        self.allocationMethod='adaptive' #adaptive for dynamic allocation, uniform for equal allocation    
        try:
    ...
    def is_WE_basis(self,pcoords):
        isBasis=np.logical_and(pcoords[:,0]>self.WEbasisp1_min,pcoords[:,0]<self.WEbasisp1_max)
        return isBasis
            ...
            try:
                ...
                if e:
                    ...
                    for iS in range(nS):
                        ...
                        pcoord0List=np.append(pcoord0List,np.expand_dims(pcoord[iS,0,:],0),axis=0)
                        pcoord1List=np.append(pcoord1List,np.expand_dims(pcoord[iS,self.pcoord_len-1,:],0),axis=0)
                        nSeg=nSeg+1
                dataIn.close()
            except:
        ...
        while nSeg>0:
            ...
            for iF in range(self.nF):
                ...
                try:
                    ...
                    sys.stdout.write('no segments in '+fileName+str(sys.exc_info()[0])+'\n')
            if nSeg>0:
                numSegments=np.append(numSegments,nSeg)
                sys.stdout.write('Iteration '+str(n_iter)+' has '+str(nSeg)+' segments...\n')
            n_iter=n_iter+1    
    ...
    def get_transition_data(self,n_lag): #get segment history data at lag time n_lag from current iter
        if n_lag>self.n_iter:
            sys.stdout.write('too much lag for iter... n_lag reduced to: '+str(self.n_iter)+'\n')
            n_lag=self.n_iter
        ...
        for iS in range(self.nSeg):
            try:
                if iS==0:
                    ...
                    dset = dataIn[dsetName]
                    coords_current = dset[:]
                    dsetName="/iterations/iter_%08d/auxdata/coord" % int(self.n_iter-n_lag)
                    dset = dataIn[dsetName]
                    coords_lagged = dset[:]
                    ...
                    dset = dataIn[dsetName]
                    coords_lagged = dset[:]
                coordPairList[iS,:,:,1]=coords_current[self.segindList[iS],1,:,:]
                if warpList[iS]==0:
                    coordPairList[iS,:,:,0]=coords_lagged[segindList_lagged[iS],0,:,:]
                elif warpList[iS]>0:
                    prewarpedStructures[nWarped,:,:]=coords_lagged[segindList_lagged[iS],0,:,:]
                    coordPairList[iS,:,:,0]=self.basis_coords
                    nWarped=nWarped+1
            except:
                weightList_lagged[iS]=0.0
                weightList[iS]=0.0 #set transitions without structures to zero weight
        ...
        for iW in range(nWarped):
            ...
            try:
                ...
                sys.stdout.write('    segment '+str(indWarped[iW])+' warped '+str(nW)+' times\n')
            except:
                sys.stdout.write('    segment '+str(indWarped[iW])+' warped 1 time\n')
            transitionWeights=np.append(transitionWeights,self.weight_histories[indWarped[iW],iterWarped])
            departureWeights=np.append(departureWeights,self.weight_histories[indWarped[iW],n_lag])        
        self.coordPairList=coordPairList
        self.transitionWeights=transitionWeights
        self.departureWeights=departureWeights

    def get_transition_data_lag0(self): #get segment history data at lag time n_lag from current iter
        weightList=self.weightList
        coordPairList=np.zeros((self.nSeg,self.nAtoms,3,2))
        for iS in range(self.nSeg):
            try:
                if iS==0:
                    ...
                    dset = dataIn[dsetName]
                    coords = dset[:]
                coordPairList[iS,:,:,1]=coords[self.segindList[iS],self.pcoord_len-1,:,:]
    ...
    def get_warps_from_parent(self,first_iter,last_iter): #get all warps and weights over set of iterations
        warpedWeights=[]
    ...
    def get_warps_from_pcoord(self,first_iter,last_iter): #get all warps and weights over set of iterations
        warpedWeights=[]
        for iS in range(first_iter,last_iter+1):
            ...
            warpedWeights.append(self.weightList[warpList])
            meanJ=np.mean(self.weightList[warpList])/self.tau/np.sum(self.weightList)
    ...
    def get_direct_target_flux(self,first_iter,last_iter,window):
        ...
        dsetName = '/s'+str(first_iter)+'_e'+str(last_iter)+'_w'+str(window)+'/Jdirect'
        e = dsetName in f
        if not e:
            ...
            dsetP[:] = Jdirect
            dsetName = '/s'+str(first_iter)+'_e'+str(last_iter)+'_w'+str(window)+'/JdirectTimes'
    ...
    def get_seg_histories(self,n_hist):
        if n_hist>self.n_iter:
            sys.stdout.write('we have too much history... n_hist reduced to: '+str(self.n_iter)+'\n')
            n_hist=self.n_iter
        self.n_hist=n_hist
        ...
        for iS in range(self.nSeg):
            if iS%100==0:
                sys.stdout.write('        getting history for iteration '+str(self.n_iter)+' segment '+str(iS)+'...\n')
            if iS==0:
                westFile=self.fileList[self.westList[iS]]
                dataIn=h5py.File(westFile,'r')
            elif self.westList[iS] != self.westList[iS-1]:
            ...
            for iH in range(1,self.n_hist+1):
                ...
                if indCurrentSeg<0 and warped==0:
                    sys.stdout.write('Segment '+str(iS)+' warped last iter: History must end NOW!\n')
                    warped=1
                elif indCurrentSeg>=0 and warped==0:
                    dsetName = "/iterations/iter_%08d/seg_index" % int(self.n_iter-iH+1)
                    dset=dataIn[dsetName]
                    seg_histories[iS,iH]=dset[indCurrentSeg][1]
                    weight_histories[iS,iH-1]=dset[indCurrentSeg][0]
                    if seg_histories[iS,iH]<0:
                        sys.stdout.write('            segment '+str(iS)+' warped '+str(iH)+' iters ago\n')
    ...
    def collect_iter_coordinates(self): #grab coordinates from WE traj_segs folder
        ...
        for iS in range(self.nSeg):
            ...
            coord0=np.squeeze(md.load(trajpath+'/parent.rst7',top=self.reference_structure.topology)._xyz)
            coord1=np.squeeze(md.load(trajpath+'/seg.rst7',top=self.reference_structure.topology)._xyz)
            ...
            try:
                if iS > 0:
                    if self.westList[iS] != self.westList[iS-1] and iS<nS-1:
                        dsetName="/iterations/iter_%08d/auxdata/coord" % int(self.n_iter)
                        try:
                            dset = dataIn.create_dataset(dsetName, np.shape(coords[:-1,:,:,:]))
                            dset[:] = coords[:-1,:,:,:]
                        except:
                            del dataIn[dsetName]
                            dset = dataIn.create_dataset(dsetName, np.shape(coords[:-1,:,:,:]))
                            dset[:] = coords[:-1,:,:,:]
                            sys.stdout.write('coords exist for iteration '+str(n_iter)+' overwritten\n')
                        dataIn.close()
                        coords=np.zeros((0,2,self.nAtoms,3))  
                        coords=np.append(coords,coordT,axis=0)
                        dataIn=h5py.File(westFile,'a')
                        ...
                        try:
                            ...
                            dset = dataIn.create_dataset(dsetName, np.shape(coords))
                            dset[:] = coords
                            sys.stdout.write('coords exist for iteration '+str(self.n_iter)+' overwritten\n')
                        dataIn.close()
            except:
                sys.stdout.write('error collecting coordinates from '+WEfolder+' , iter '+str(self.n_iter)+' segment '+str(self.segindList[iS])+'\n')
    ...
    def get_iter_coordinates(self): #get iteration final coordinates
        ...
        for iS in range(self.nSeg):
            try:
                ...
                sys.stdout.write('error getting coordinates from '+self.fileList[self.westList[iS]]+' , iter '+str(self.n_iter)+' segment '+str(self.segindList[iS])+'\n')
    ...
    def get_coordinates(self,first_iter,last_iter):
        ...
        for iter in iters:
            if iter%50==0:
                sys.stdout.write('    gathering structures from iteration '+str(iter)+'...\n')
    ...
    def get_traj_coordinates(self,from_iter,traj_length):
        if traj_length>from_iter:
            ...
            sys.stdout.write('trajectory length too long: set to '+str(traj_length)+'\n')
        ...
        for i in range(from_iter,from_iter-traj_length,-1):
            sys.stdout.write('    gathering structures from iteration '+str(i)+'...\n')
            self.get_iter_data(i)
            self.get_iter_coordinates()
            seg_history=self.seg_histories[:,iH]
            seg_history_index=np.zeros(nS).astype(int)
            last_index=np.zeros(nS).astype(int) #list of final iterations for warped particles
            for iS in range(nS):
                if seg_history[iS]>0:
                    seg_history_index[iS]=np.where(np.logical_and(self.segindList==seg_history[iS],self.westList==westList[iS]))[0][0] #segment indexes are local to the westfile, not global for the analysis set
                    last_index[iS]=0
    ...
    def dimReduce(self):
        ...
        if self.dimReduceMethod=='pca':
            ...
            self.coordinates=coor.pca(data, dim=-1, var_cutoff=0.95, mean=None, stride=1, skip=0)
        ...
        if self.dimReduceMethod=='vamp':
            ...
            self.coordinates=coor.vamp(data, lag=self.vamp_lag, dim=self.vamp_dim, scaling=None, right=False, stride=1, skip=0)
            self.ndim=self.coordinates.dimension()
    ...
    def cluster_coordinates(self,n_clusters):
        ...
        if self.dimReduceMethod=='none':
            if self.nAtoms>1:
                self.clusters=coor.cluster_kmeans([self.coordSet.reshape(nC,3*self.nAtoms)],k=n_clusters,metric='minRMSD') #,max_iter=100)
            elif self.nAtoms==1:
                self.clusters=coor.cluster_kmeans([self.coordSet.reshape(nC,3*self.nAtoms)],k=n_clusters,metric='euclidean')
        if self.dimReduceMethod=='pca' or self.dimReduceMethod=='vamp':
            self.clusters=coor.cluster_kmeans(self.coordinates.get_output(),k=n_clusters,metric='euclidean') #,max_iter=100)
        self.clusterFile=self.modelName+'_clusters_s'+str(self.first_iter)+'_e'+str(self.last_iter)+'_nC'+str(self.n_clusters)+'.h5'
    ...
    def get_iter_fluxMatrix(self,n_iter):
        ...
        cluster0=self.clusters.assign(self.reduceCoordinates(self.coordPairList[:,:,:,0]))
        cluster1=self.clusters.assign(self.reduceCoordinates(self.coordPairList[:,:,:,1]))
        ...
        indBasis0=np.where(self.is_WE_basis(self.pcoord0List)) #needs to fit definition of target in WE
        ...
        fluxMatrix=coo_matrix((self.transitionWeights,(cluster0,cluster1)),shape=(self.n_clusters+2,self.n_clusters+2)).todense()
        return fluxMatrix
    ...
    def get_pcoord1D_fluxMatrix(self,n_lag,first_iter,last_iter,binbounds):
        ...
        nI=0
        f=h5py.File(self.modelName+'.h5','a')
        dsetName = '/s'+str(first_iter)+'_e'+str(last_iter)+'_lag'+str(n_lag)+'_b'+str(nBins)+'/pcoord1D_fluxMatrix'
        e = dsetName in f
        if not e:
            ...
            for iS in range(first_iter+1,last_iter+1):
                ...
                fluxMatrixI=fluxMatrixI/np.sum(self.weightList) #correct for multiple trees
    ...
    def get_fluxMatrix(self,n_lag,first_iter,last_iter):
        ...
        nI=0
        fileName=self.modelName+'_s'+str(first_iter)+'_e'+str(last_iter)+'_lag'+str(n_lag)+'_clust'+str(self.n_clusters)
    ...
    def organize_fluxMatrix(self): #processes raw fluxMatrix
        ...
        while nTraps>0:
            ...
            for iC in range(self.n_clusters):
                ...
                statesum=np.sum(fluxMatrixTraps[:,iC])+np.sum(fluxMatrixTraps[iC,:])
                if statesum==0.0:
                    indData[iC]=0
                if statesum>0:
        ...
        fluxMatrix=fluxMatrix[:,indp1]
        self.fluxMatrix=fluxMatrix/np.sum(fluxMatrix) #average weight transitioning or staying put should be 1
    ...
    def get_model_clusters(self): #define new clusters from organized flux matrix corresponding to model
        ...
        if self.dimReduceMethod=='none':
            ...
            clustercenters[self.n_clusters:,:]=self.reduceCoordinates(coords) #add in basis and target
            self.model_clusters=clustering.AssignCenters(self.reduceCoordinates(clustercenters[self.originalClusters,:]), metric='euclidean', stride=1, n_jobs=None, skip=0)
        if self.dimReduceMethod=='pca' or self.dimReduceMethod=='vamp':
            coords=np.array([self.reduceCoordinates(self.basis_coords),self.reduceCoordinates(self.reference_coord)])
            clustercenters[self.n_clusters:,:]=np.squeeze(coords) #add in basis and target
            self.model_clusters=clustering.AssignCenters(clustercenters[self.originalClusters,:], metric='euclidean', stride=1, n_jobs=None, skip=0)
    ...
    def get_Tmatrix(self):
        ...
        sinkRates[0,self.indBasis]=1./self.indBasis.size
        Mss=Mt.copy()
    ...
    def get_steady_state_target_flux(self):
        ...
        for j in range(nTargets):
            ...
            Jt=Jt+np.sum(np.multiply(pSS[indNotTargets],np.squeeze(np.array(Mss[indNotTargets,jj*np.ones_like(indNotTargets)]))))
        self.JtargetSS=Jt/self.lagtime

    def evolve_probability(self,nEvolve,nStore): #iterate nEvolve times, storing every nStore iterations, initial condition at basis
        ...
        for i in range(nEvolve):
            ...
            if i%nStore==0:
                ...
                try:
                    plt.plot(binCenters,np.squeeze(pSS),'-',color=plt.cm.Greys(float(i)/float(nEvolve)))
                    plt.yscale('log')
                except:
                    try:
                        plt.plot(binCenters,pSS.A1,'-',color=plt.cm.Greys(float(i)/float(nEvolve)))
                    except:
                        pass
    ...
    def evolve_probability2(self,nEvolve,nStore): #iterate nEvolve times, storing every nStore iterations, initial condition spread for everything at RMSD higher than basis
        nIterations=np.ceil(nEvolve/nStore).astype(int)+1
        self.nEvolve=nEvolve
        self.nStore=nStore
        Mss=self.Tmatrix
        ...
        binCenters=self.binCenters
        probBasis=np.zeros((1,self.nBins))
        probBasis[0,self.indBasis[0]:]=1.0 #assign initial probability to everything at RMSD higher than the basis, for case when nothing observed leaving exact basis
    ...
    def evolve_probability_from_initial(self,p0,nEvolve,nStore): #iterate nEvolve times, storing every nStore iterations, initial condition provided
        nIterations=np.ceil(nEvolve/nStore).astype(int)+1
        self.nEvolve=nEvolve
        self.nStore=nStore
        Mss=self.Tmatrix
    ...
    def plot_flux_committor(self,nwin):
        ...
        ax.plot(q_av[indMinus],-np.squeeze(Jq_av[indMinus]),'<',color='black',linewidth=2,markersize=10,label='flux toward unfolded')
        ax.plot(q_av[indPlus],np.squeeze(Jq_av[indPlus]),'>',color='black',linewidth=2,markersize=10,label='flux toward folded')
        plt.yscale('log')
        plt.xscale('log')
    ...
    def plot_flux(self):
        ...
        ax.plot(binCenters[indPlus],np.squeeze(J[indPlus]),'ko',linewidth=2,markersize=10,label='flux toward folded')#,color=plt.cm.Greys(float(iStep)/float(nStepFrames)))
        ax.plot(binCenters[indMinus],-np.squeeze(J[indMinus]),'ro',linewidth=2,markersize=10,label='flux toward unfolded')#,color=plt.cm.Reds(float(iStep)/float(nStepFrames)))
        plt.yscale('log')
        plt.ylabel('flux (weight/second)',fontsize=12)
        plt.xlabel('RMSD ($\AA$)',fontsize=12)
        plt.title('Flux Run 1-30 Iter '+str(self.first_iter)+'-'+str(self.last_iter))
        plt.legend(loc='lower right')
        plt.pause(1)
        fig.savefig('flux_s'+str(self.first_iter)+'_e'+str(self.last_iter)+'.png')
    ...
    def evolve_target_flux(self):
        ...
        for iT in range(nT):
            ...
            for j in range(nTargets):
                jj=self.indTargets[j]
                Jt=Jt+np.sum(np.multiply(probTransient[iT,indNotTargets],Mss[indNotTargets,jj*np.ones_like(indNotTargets)]))
            Jtarget[iT]=Jt
    ...
    def get_hflux(self,conv):
        ...
        while dconvh>convh or dconvf>convf and N<max_iters:
            ...
            for i in range(self.nBins):
                ...
                for j in range(nTargets):
                    jj=self.indTargets[j]
                    Jt=Jt+np.sum(np.multiply(Pt[0,indNotTargets],Mt[indNotTargets,jj*np.ones_like(indNotTargets)]))
                f[i,0]=Jt/self.tau
            fTotal=fTotal+f
            ...
            sys.stdout.write('N='+str(N)+' dh: '+str(dconvh)+' df: '+str(dconvf)+' Jss:'+str(fSS)+'\n')
            hp=ht.copy()
            fSSp=fSS
            self.h=ht.copy()
    ...
    def get_model_steady_state_aristoffian(self):
        ...
        if self.binMethod=='adaptive':
            ...
            if khbins_centers_unique.size != nB:
                khbins=np.squeeze(np.linspace(np.min(self.kh),np.max(self.kh),nB+1)) #equal spacing if not enough for k-means
                khbins_centers=0.5*(khbins[1:]+khbins[0:-1])
                self.kh_clusters=clustering.AssignCenters(khbins_centers[:,np.newaxis], metric='euclidean', stride=1, n_jobs=None, skip=0)
        elif self.binMethod=='uniform':
            khbins=np.squeeze(np.linspace(np.min(self.kh),np.max(self.kh),nB+1)) #equal spacing if not enough for k-means
            khbins_centers=0.5*(khbins[1:]+khbins[0:-1])
            self.kh_clusters=clustering.AssignCenters(khbins_centers[:,np.newaxis], metric='euclidean', stride=1, n_jobs=None, skip=0)
        elif self.binMethod=='log_uniform':
            transformedBins=np.geomspace(np.abs(np.min(self.kh))/np.max(self.kh),1.0+2.0*np.abs(np.min(self.kh))/np.max(self.kh),self.nB+1) 
            khbins_binEdges_log=transformedBins*np.max(self.kh)-2.0*np.abs(np.min(self.kh))  
            ...
            try:
                ...
                sys.stdout.write('khbins (khbins_binCenters.dat) not found: initializing\n')
        ...
        for v in range(self.nB):
            ...
            for u in range(self.nB):
                ...
                for p in indBinv[0]:
                    for q in indBinu[0]:
                        rhov[u,v]=rhov[u,v]+self.alloc[v]*(self.pSS[p]/wv)*self.Tmatrix[p,q]
        rho=np.sum(rhov,1)
        pOccupied=1.-np.power(1.-rho,self.nW)
        nOccupied=nB-np.sum(np.power(1.-rho,self.nW))
        ...
        for i in range(nB):
            ...
            bin_mutV[i]=((wi**2)/(nT[i]))*np.sum(np.multiply(self.pSS[indBin]/wi,self.varh[indBin]))
            bin_selV[i]=((wi**2)/(nT[i]))*np.sum(np.multiply(self.pSS[indBin]/wi,np.power(self.kh[indBin],2))-np.power(np.multiply(self.pSS[indBin]/wi,self.kh[indBin]),2))
        self.binObjective=np.sum(bin_mutV+bin_selV)
        binEdges=np.zeros(self.nB+1)
        binEdges[0]=-np.inf
        binEdges[-1]=np.inf
        ind=np.argsort(self.kh_clusters.clustercenters[:,0]) #note sorting makes kh_clusters indexes different        
        self.khbins_binCenters=self.kh_clusters.clustercenters[ind,0]
        binEdges[1:-1]=0.5*(self.khbins_binCenters[1:]+self.khbins_binCenters[0:-1])
        self.khbins_binEdges=binEdges
        #self.walkers_per_bin=walkers[ind]
    ...
    def get_bin_kh_var(self,x):
        nB=self.nB
        self.kh_clusters=clustering.AssignCenters(x[:,np.newaxis], metric='euclidean', stride=1, n_jobs=None, skip=0)
        dtraj_kh_clusters=self.kh_clusters.assign(self.kh)
        #alloc=np.zeros(nB) #get bin objective function, value and allocation over set of bins
        bin_kh_var=np.zeros(nB)
        for i in range(nB):
...
#        else:
        ...
        if khList_unique.size>2.0*nB and self.binMethod=='adaptive':
            self.kh_clusters=coor.cluster_kmeans(khList_unique,k=nB,metric='euclidean')
            khbins_centers=self.kh_clusters.clustercenters[:,0]
            khbins_centers_unique,ind_unique=np.unique(khbins_centers,return_index=True)
            if khbins_centers_unique.size != nB:
                khbins=np.squeeze(np.linspace(np.min(self.kh),np.max(self.kh),nB+1)) #equal spacing if not enough for k-means
            ...
            transformedBins=np.geomspace(np.abs(np.min(self.kh))/np.max(self.kh),1.0+2.0*np.abs(np.min(self.kh))/np.max(self.kh),self.nB)
            khbins_binEdges_log=transformedBins*np.max(self.kh)-2.0*np.abs(np.min(self.kh))
            khbins=khbins_binEdges_log #equal log-spacing
            khbins_centers=0.5*(khbins[1:]+khbins[0:-1])
            self.kh_clusters=clustering.AssignCenters(khbins_centers[:,np.newaxis], metric='euclidean', stride=1, n_jobs=None, skip=0)
    ...
    def write_iter_kh_pcoord(self): #grab coordinates from WE traj_segs folder
        ...
        westFile=self.fileList[self.westList[0]]
        dataIn=h5py.File(westFile,'a')
        pcoords=np.zeros((0,2,2)) #this is explicitly set up for p1 (target,basis def) and p2 (kh-aristoffian)
        for iS in range(self.nSeg):
            westFile=self.fileList[self.westList[iS]]
            ...
            try:
                if iS > 0:
                    if self.westList[iS] != self.westList[iS-1] and iS<nS-1:
                        ...
                        del dataIn[dsetName]
                        dset = dataIn.create_dataset(dsetName, np.shape(pcoords[:-1,:,:]))
                        dset[:] = pcoords[:-1,:,:]
                        dataIn.close()
                        pcoords=np.zeros((0,2,2)) #this is explicitly set up for p1 (target,basis def) and p2 (kh-aristoffian) 
                        pcoords=np.append(pcoords,pcoord,axis=0)
                        ...
                        dset[:] = pcoords
                        sys.stdout.write('pcoords for iteration '+str(self.n_iter)+' overwritten\n')
                        dataIn.close()
            except:
                sys.stdout.write('error overwriting pcoord from '+westFile+' , iter '+str(self.n_iter)+' segment '+str(self.segindList[iS])+'\n')
    ...
    def plot_committor(self):
        ...
        plt.pause(1)
        fig.savefig(self.modelName+'_s'+str(self.first_iter)+'_e'+str(self.last_iter)+'committor.png')
